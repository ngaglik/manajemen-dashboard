...
export default defineComponent({
  setup() {
    ...
    const fetchData = async (page = 1) => {
      loading.value = true;
      try {
        let url = `${Config.UrlBackend}/api/fin/prop-dashboards?page=${page}&pageSize=${pageSize.value}`;
    ...
    const deleteRow = (row: any) => {
      dialog.warning({
        ...
        content: `Hapus grafik "${row.name}"? Semua indikator di dalamnya juga akan terhapus.`,
        positiveText: "Hapus",
        negativeText: "Batal",
        onPositiveClick: async () => {
          try {
            const res = (await apiFetch(
              `${Config.UrlBackend}/api/fin/prop-dashboards/${row.id}`,
              { method: "DELETE" },
            )) as Response;
            if (!res || !res.ok) {
    ...
    const columns = [
      {
        ...
        render(row: any) {
          return h("div", { style: { display: "flex", gap: "6px" } }, [
            h(NButton, { size: "small", type: "primary", onClick: () => openModal(row) },
              { default: () => "Edit" }),
            h(NButton, { size: "small", type: "info", onClick: () => openItemsModal(row) },
              { default: () => "Indikator" }),
            h(NButton, { size: "small", type: "error", onClick: () => deleteRow(row) },
              { default: () => "Hapus" }),
          ]);
        },
      },
      { title: "Nama Grafik", key: "name" },
      { title: "Tahun", key: "year", width: 80 },
      {
        ...
        width: 100,
        render: (row: any) =>
          h(NTag, { type: row.itemCount > 0 ? "success" : "default", size: "small" },
    ...
    const openModal = (row?: any) => {
      if (row) {
        ...
        formData.value = { id: null, name: "", year: currentYear, isActive: true };
      }
    ...
    const submitModal = async () => {
      if (!formData.value.name.trim()) {
        message.warning("Nama grafik wajib diisi");
        return;
      }
      ...
      try {
        ...
        const url = isEditMode.value
          ? `${Config.UrlBackend}/api/fin/prop-dashboards/${formData.value.id}`
          : `${Config.UrlBackend}/api/fin/prop-dashboards`;
    ...
    // ══════════════════════════════════════════════════════════════
    // MODAL KELOLA INDIKATOR (one-to-many items)
    // ══════════════════════════════════════════════════════════════
    ...
    // Daftar item yang sedang diedit: { indicatorId, label, sortOrder }
    ...
    const addItemToList = () => {
      ...
      if (editItems.value.some((x) => x.indicatorId === selectedIndicatorId.value)) {
    ...
    const moveItem = (idx: number, dir: -1 | 1) => {
      ...
      [editItems.value[idx], editItems.value[target]] =
        [editItems.value[target], editItems.value[idx]];
      editItems.value.forEach((x, i) => { x.sortOrder = i + 1; });
    };
    ...
    const saveItems = async () => {
      ...
      try {
        ...
        const res = (await apiFetch(
          `${Config.UrlBackend}/api/fin/prop-dashboards/${activeDashboard.value.id}/items`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
    ...
    return {
      ...
      // modal dashboard
      isModalOpen, isEditMode, modalLoading, formData,
      openModal, submitModal,
      // modal items
      isItemsModalOpen, itemsModalLoading, itemsSaveLoading,
      activeDashboard, editItems, selectedIndicatorId,
      indicatorOptions, openItemsModal, addItemToList,
      removeItemFromList, moveItem, saveItems, getIndicatorName,
      currentYear,
    };
  },
});
