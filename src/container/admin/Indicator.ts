...
export default defineComponent({
  setup() {
    ...
    const fetchGroupIdOptions = async () => {
      try {
        ...
        groupIdOptions.value = (result.data || result).map((item: any) => ({
          label: item.name,
          value: item.id,
        }));
      } catch (error) {
        console.error(error);
        message.error("Gagal memuat option data");
      }
    };
    ...
    const fetchData = async (page = 1) => {
      ...
      const response = (await apiFetch(
        Config.UrlBackend +
          `/api/fin/indicators?page=${page}&pageSize=${pageSize.value}&search=${inputSearch.value}`,
        {
          method: "GET",
        },
      )) as Response;
    ...
    const criteriaOptions = [
      { label: "Bulan Terakhir", value: "last_month" },
      { label: "Akumulasi", value: "accumulation" },
    ];
    ...
    const deleteData = async (row: any) => {
      dialog.warning({
        ...
        onPositiveClick: async () => {
          ...
          try {
            const response = (await apiFetch(
              `${Config.UrlBackend}/api/fin/indicators/${row.id}`,
              {
                method: "DELETE",
                headers: {
    ...
    const submitForm = async () => {
      if (isEditMode.value) {
        ...
        try {
          const response = (await apiFetch(
            `${Config.UrlBackend}/api/fin/indicators/${formData.value.id}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
        ...
        try {
          const response = (await apiFetch(
            `${Config.UrlBackend}/api/fin/indicators`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
    ...
    const columns = [
      ...
      { title: "Group", key: "groupName" },
      {
        ...
        width: 140,
        render(row: any) {
          return row.criteria === "accumulation"
            ? "📊 Akumulasi"
            : "📅 Bulan Terakhir";
        },
      },
